...
import { parseSubagentFinished, parseSubagentStarted } from "../extensions/jpi-sidebar/subagents-bus.ts";
...
test("parseSubagentStarted requires a non-empty string id and passes through optional fields", () => {
  ...
  assert.deepEqual(parseSubagentStarted({ id: "a1" }), { id: "a1", type: undefined, description: undefined });
...
test("parseSubagentFinished tolerates a garbage tokens field and only accepts a fully-numeric one", () => {
  ...
  assert.deepEqual(full, { id: "a1", type: undefined, description: undefined, toolUses: 4, durationMs: 1234, tokens: { input: 10, output: 20, total: 30 } });
